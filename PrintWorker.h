...
#define BEGIN  3
//定义间隔时间，就是从BEGIN秒开始之后，每隔PERIOD秒请求一次打印
#define PERIOD  3
namespace Printer_Lsh{
	...
	struct PrintInfo 
	{
		PrintInfo(){
			...
			//iPrinting = 0;
			strUrl = "http://demo.host12.v-ct.com/terminal_api?sn=123";//默认使用这个地址请求数据
...
// 		}
	...
	public:
		...
		*/
		...
		static PrintRecorder GetLog(size_t index);
	...
	private:
		...
		static std::vector<PrintRecorder> s_Log; //记录打印的日志
	};
